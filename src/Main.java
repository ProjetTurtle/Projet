...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("                                                                     BIENVENUE SUR ROBOT TURTLE" + "\n");
		...
		Joueur[] listeJoueur = new Joueur[] {joueur1,joueur2,joueur3,joueur4};
		
		initialisation.joueurs(joueur1);
		...
		// attention lorsqu'un joueur attérit sur un diament, il ne peut plus jouer son
		// tour
		...
		while (fin.getNombreDeJoueurActif() > 1) {
			...
			System.out.println(
					joueur.getNom(tour.getIndice()) + " est position : " + joueur.getPosition(tour.getIndice()));

			System.out.println(
					joueur.getNom(tour.getIndice()) + " est orientation : " + joueur.getOrientation(tour.getIndice()));

			System.out.println("\n");

			System.out.println(" C'est au tour de : " + joueur.getNom(tour.getIndice()) + ", il joue la tortue "
					+ joueur.getNumero(tour.getIndice()));
			...
			if (reponse.equals("M")) {
				...
				System.out.println(" Sur quelle case voulez-vous construire ce mur ?");

				int numeroCase = scanner.nextInt();

				action.construire(pierreOuGlace, numeroCase, plateau, joueur, tour);
			...
			if (reponse.equals("C")) {
				...
				if (joueur.getDeck(tour.getIndice()).size() == 0) {
					...
					for (int i = 0; i < 5; i++) {
						...
						System.out.println(joueur.getDeck(tour.getIndice()).remove(0));
